...
function query_users () {
    echo ""
    echo "Execute beeline query for <$(($2-$1+1))> users"
    echo ""
    START=$1
    FINISH=$2
    for i in `seq -f "%02g" $START $FINISH`; do
           username=user$i
...
function query_groups () {
    echo ""
    echo "[INFO] Execute beeline query for <$(($2-$1+1))> users"
    echo ""
    FIRST=$1
    LAST=$2
...
if [ $1 == "Stress" ]; then
        ...
        for i in `seq -f "%02g" $RUNTIMES`; do
            ...
            echo "[INFO] Test $i"
            echo ""
            echo "[INFO] Stressing Users"
            query_users "0" "$(($2-1))"
            echo ""
            echo "[INFO] Stabilizing the system, sleeping $SLEEP seconds... please wait..."
            sleep $SLEEP
            echo ""
            echo "[INFO] Stressing Groups"
